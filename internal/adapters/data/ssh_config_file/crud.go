...
// serverExistsInHosts checks if a server with the given alias already exists in the provided host list.
func (r *Repository) serverExistsInHosts(hosts []*ssh_config.Host, alias string) bool {
	return r.findHostByAliasInHosts(hosts, alias) != nil
}
...
// findHostByAliasInHosts finds a host by its alias in the provided host list.
func (r *Repository) findHostByAliasInHosts(hosts []*ssh_config.Host, alias string) *ssh_config.Host {
